...
public class ModuleButton extends AbstractButton {
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        Color accentFaded = Render2DEngine.applyOpacity(accent, 0.12f);
        Color hoverBg = new Color(255, 255, 255, 10);
        Color itemBg = new Color(0, 0, 0, 0);
        Color enabledText = new Color(235, 235, 240);
        Color disabledText = new Color(150, 150, 160);
        Color arrowColor = new Color(100, 100, 115);

        if (isOpen()) {
            Render2DEngine.drawRoundedRect(context.getMatrices(), x + 3, y, width - 6, height + (float) getElementsHeight() + 2, 4f, new Color(20, 20, 26, 200));
        ...
        if (hovered && !isOpen()) {
            Render2DEngine.drawRoundedRect(context.getMatrices(), x + 3, y + 0.5f, width - 6, height - 1, 3f, hoverBg);
        }

        if (animation > 0.05f) {
            Render2DEngine.drawRoundedRect(context.getMatrices(), x + 3, y + 0.5f, width - 6, height - 1, 3f, Render2DEngine.applyOpacity(accent, 0.08f * animation));
            Render2DEngine.drawRoundedRect(context.getMatrices(), x + 3, y + 0.5f, 2f, height - 1, 2f, Render2DEngine.applyOpacity(accent, animation));
        }

        Color enabledText = new Color(235, 235, 240);
        Color disabledText = new Color(150, 150, 160);
        Color arrowColor = new Color(100, 100, 115);
        Color accentText = HudEditor.getColor(0);
