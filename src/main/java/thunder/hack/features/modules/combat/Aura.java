...
public class Aura extends Module {
    ...
    public void onUpdate(PlayerUpdateEvent e) {
        ...
        if (mc.field_1724.method_6115() && pauseWhileEating.getValue()) return;
    ...
    private boolean autoCrit() {
        ...
        if (mc.field_1724.method_5771() || mc.field_1724.method_5869()) return true;

        if (!mc.field_1690.field_1903.method_1434() && isAboveWater()) return true;

        if (mc.field_1724.field_6017 > 1 && mc.field_1724.field_6017 < 1.14) return false;
    ...
    private void renderOrbitEsp(class_1297 target) {
        ...
        for (int i = 0; i < totalBeads; i++) {
            ...
            // Hạt lớn hơn ở vị trí đầu xích, nhỏ hơn ở cuối
            float beadSize = size * (0.6f + 0.4f * progress);

            class_238 box = new class_238(
                x - beadSize, cy - beadSize, z - beadSize,
                x + beadSize, cy + beadSize, z + beadSize
            );
            Render3DEngine.FILLED_QUEUE.add(new Render3DEngine.FillAction(box, fill));

            // Chỉ vẽ outline cho hạt lớn (mỗi 3 hạt 1 cái) để trông giống mắt xích
            if (i % 3 == 0) {
    ...
    private boolean skipEntity(class_1297 entity) {
        ...
        if (entity instanceof class_1657 player) {
            ...
            if (player == mc.field_1724 || Managers.FRIEND.isFriend(player)) return true;
            if (player.method_7337() && ignoreCreative.getValue()) return true;
            if (player.method_6096() == 0 && ignoreNaked.getValue()) return true;
            if (player.method_5767() && ignoreInvisible.getValue()) return true;
    ...
    private boolean skipNotSelected(class_1297 entity) {
        ...
        if (entity instanceof class_1588 he) {
            ...
            if (onlyAngry.getValue()) return !he.method_7076(mc.field_1724);
        }
    ...
    public enum ESP { Off, ThunderHack, NurikZapen, CelkaPasta, ThunderHackV2, Orbit }
