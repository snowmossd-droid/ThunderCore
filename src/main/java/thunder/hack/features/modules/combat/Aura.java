...
public class Aura extends Module {
    ...
    public final Setting<SettingGroup> crossGroup = new Setting<>("HolyCrossSettings", new SettingGroup(false, 0), v -> esp.is(ESP.HolyCross));
    public final Setting<Float> crossSize = new Setting<>("Size", 0.8f, 0.3f, 1.8f, v -> esp.is(ESP.HolyCross)).addToGroup(crossGroup);
    public final Setting<Float> crossSpeed = new Setting<>("Speed", 2.0f, 0.5f, 10.0f, v -> esp.is(ESP.HolyCross)).addToGroup(crossGroup);
    public final Setting<Float> crossThickness = new Setting<>("Thickness", 0.08f, 0.03f, 0.2f, v -> esp.is(ESP.HolyCross)).addToGroup(crossGroup);
    public final Setting<Boolean> crossRainbow = new Setting<>("Rainbow", true, v -> esp.is(ESP.HolyCross)).addToGroup(crossGroup);
    public final Setting<Integer> crossOutline = new Setting<>("Outline", 2, 0, 4, v -> esp.is(ESP.HolyCross)).addToGroup(crossGroup);
    ...
    public void onUpdate(PlayerUpdateEvent e) {
        ...
        crossAngle += crossSpeed.getValue() * 0.05;
        if (crossAngle > Math.PI * 2) crossAngle -= Math.PI * 2;
    }
    ...
    public void onRender3D(MatrixStack stack) {
        ...
        switch (esp.getValue()) {
            ...
            case HolyCross -> renderHolyCrossEsp(target);
        }
    ...
    private void renderHolyCrossEsp(Entity target) {
        double cx = target.getX();
        double cy = target.getY() + target.getBoundingBox().getLengthY() / 2.0;
        double cz = target.getZ();
        float size = crossSize.getValue();
        float thickness = crossThickness.getValue();
        int outline = crossOutline.getValue();
        
        double angleRad = crossAngle;
        float cos = (float) Math.cos(angleRad);
        float sin = (float) Math.sin(angleRad);
        
        double verticalX1 = cx - thickness;
        double verticalX2 = cx + thickness;
        double verticalY1 = cy - size;
        double verticalY2 = cy + size;
        double verticalZ1 = cz - thickness;
        double verticalZ2 = cz + thickness;
        
        Box verticalBar = new Box(verticalX1, verticalY1, verticalZ1, verticalX2, verticalY2, verticalZ2);
        
        double horizontalX1 = cx - size;
        double horizontalX2 = cx + size;
        double horizontalY1 = cy - thickness;
        double horizontalY2 = cy + thickness;
        double horizontalZ1 = cz - thickness;
        double horizontalZ2 = cz + thickness;
        
        Box horizontalBar = new Box(horizontalX1, horizontalY1, horizontalZ1, horizontalX2, horizontalY2, horizontalZ2);
        
        Color color;
        if (crossRainbow.getValue()) {
            float hue = (float) ((crossAngle * 2) % 360) / 360f;
            color = Color.getHSBColor(hue, 1f, 1f);
        } else {
            color = HudEditor.getColor(0);
        }
        
        int alpha = 180;
        Color fill = new Color(color.getRed(), color.getGreen(), color.getBlue(), Math.max(60, alpha));
        
        double rotatedVerticalX1 = cx + (verticalX1 - cx) * cos - (verticalZ1 - cz) * sin;
        double rotatedVerticalZ1 = cz + (verticalX1 - cx) * sin + (verticalZ1 - cz) * cos;
        double rotatedVerticalX2 = cx + (verticalX2 - cx) * cos - (verticalZ2 - cz) * sin;
        double rotatedVerticalZ2 = cz + (verticalX2 - cx) * sin + (verticalZ2 - cz) * cos;
        
        Box rotatedVerticalBar = new Box(
            Math.min(rotatedVerticalX1, rotatedVerticalX2), verticalY1, Math.min(rotatedVerticalZ1, rotatedVerticalZ2),
            Math.max(rotatedVerticalX1, rotatedVerticalX2), verticalY2, Math.max(rotatedVerticalZ1, rotatedVerticalZ2)
        );
        
        double rotatedHorizontalX1 = cx + (horizontalX1 - cx) * cos - (horizontalZ1 - cz) * sin;
        double rotatedHorizontalZ1 = cz + (horizontalX1 - cx) * sin + (horizontalZ1 - cz) * cos;
        double rotatedHorizontalX2 = cx + (horizontalX2 - cx) * cos - (horizontalZ2 - cz) * sin;
        double rotatedHorizontalZ2 = cz + (horizontalX2 - cx) * sin + (horizontalZ2 - cz) * cos;
        
        Box rotatedHorizontalBar = new Box(
            Math.min(rotatedHorizontalX1, rotatedHorizontalX2), horizontalY1, Math.min(rotatedHorizontalZ1, rotatedHorizontalZ2),
            Math.max(rotatedHorizontalX1, rotatedHorizontalX2), horizontalY2, Math.max(rotatedHorizontalZ1, rotatedHorizontalZ2)
        );
        
        Render3DEngine.FILLED_QUEUE.add(new Render3DEngine.FillAction(rotatedVerticalBar, fill));
        Render3DEngine.FILLED_QUEUE.add(new Render3DEngine.FillAction(rotatedHorizontalBar, fill));
        
        if (outline > 0) {
            Render3DEngine.OUTLINE_QUEUE.add(new Render3DEngine.OutlineAction(rotatedVerticalBar, color, outline));
            Render3DEngine.OUTLINE_QUEUE.add(new Render3DEngine.OutlineAction(rotatedHorizontalBar, color, outline));
        }
    }
    ...
    public enum ESP {
        Off, ThunderHack, NurikZapen, CelkaPasta, ThunderHackV2, Orbit, HolyCross
    }
