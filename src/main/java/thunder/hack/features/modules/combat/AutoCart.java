...
public class AutoCart extends Module {
   ...
   private void handleLegitRefill(int inventorySlot, int hotbarSlot) {
      if (MovementUtility.isMoving()) {
         thunder.hack.core.InputBlocker.block("autocart_refill");
   ...
   private void clearRefillState() {
      ...
      thunder.hack.core.InputBlocker.unblock("autocart_refill");
   }
      ...
      if (!fullNullCheck() && this.mode.is(AutoCart.Mode.CrossBow) && this.cartAura.getValue()) {
         if (!this.cartAuraExecuting) {
            ...
            if (popTarget != mc.player) {
               boolean isAuraTarget = this.cartAuraTarget.getValue() && thunder.hack.features.modules.combat.Aura.target != null && popTarget == thunder.hack.features.modules.combat.Aura.target;
   ...
   private BlockPos calcBowTrajectory(float yaw) {
      if (mc.player != null && mc.world != null) {
         ...
         float power = mc.player.getItemCooldownManager().getCooldownProgress(Items.BOW, 0.0F);
         ...
         if (!mc.player.isInFluid()) {
            motionY += mc.player.getAttributeInstance(net.minecraft.entity.attribute.EntityAttributes.GRAVITY).getValue();
         }
   ...
   private boolean hasFlameEnchant(ItemStack stack) {
      return mc.world == null
         ? false
         : EnchantmentHelper.getLevel(mc.world.getRegistryManager().get(Enchantments.FLAME.getRegistryRef()).getEntry(Enchantments.FLAME).get(), stack)
            > 0;
   }
